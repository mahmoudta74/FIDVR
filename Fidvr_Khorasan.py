...
#object1.app.PrintPlain(Lines[random.randint(0, len(Lines)-1)])

# enable lines short circuit
object1.enable_short_circuits()
Motor_Buses = [
  'Ataaeshed','Bjnoordshed','Dashtjovinshad','Ferdos','Kashmaeshed','Soltanabaadshed','Terminal','Terminal_Golshahr',
  'Terminal_Pardis','Torbatshed','Tous63-2shed','abousaed','asadabad','beihagh','birjand','brdakn','bushruye','dargaz',
  'davarzan','dolatabad','farimanshed','feizabaadshed','fldkhsn','ghaen','ghazished','gholaman','ghuchanshed','golbahar',
  'gonabad','hajiabad','istghazd','jajramshed','jlgrkh','khaf','khajerabi63','kohsangi63','machnelec','mashhad',
  'mehrgan','nehbandan','neishabur','nmyshgh','rashtkhr','sabzvar','sahel','sahlabad','salehabad','sangan','sangbast',
  'sarakhs','sarayan','sarbishe','sedeh','shirvanshed','sim','solat','tabas','toossG1','tous63','trbtjam','tybaadshed',
]
...
with open('result_rms.csv', mode='w', newline='') as csv_file:
    ...
    for run in range(1):
        ...
        object1.create_short_circuit(target_name= fault_line,
                             time=1.0, fault_type= 0,
                             duration= 0.5 ,
                             fault_Res =  0,
                             Line_loc = 25, name='sc')

        
        object1.create_Switch_Event(target_name = Motors[8].GetAttribute('loc_name')+'.ElmAsm',
                            time = 1.7,
                            action = 'open',
                            name='ls1')
        ...
        object1.prepare_dynamic_sim(
                                    ...
                                    start_time =0.0, step_size= 0.02, end_time = 10)
        ...
        for i in range(len(Motors)):
            var.append(((np.array(bus_voltages[i])- 0.85) / (0.85))+ (1- np.tan(np.array(np.deg2rad(bus_angles[i])))))
